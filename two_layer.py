...
def get_co2_forcing(t, F_abrupt, dFdt):
    return F_abrupt + dFdt * t


# 4. Held Winton Model
def held_winton(t, y, C, C0, lam_lw, lam_sw, kappa, lam0, F_lw, F_sw):
    T, T_0 = y
    lam = lam_lw + lam_sw
    F = F_lw(t) + F_sw(t)
...
def run_two_layer_model(
    t_span, C, C0, lam_lw, lam_sw, kappa, lam0, F_lw, F_sw, y0=[0, 0]
):
    ...
    sol = solve_ivp(
        ...
        t_span,
        y0,
        args=(C, C0, lam_lw, lam_sw, kappa, lam0, F_lw, F_sw),
        t_eval=t_eval,
        rtol=1e-9,
        atol=1e-12,
    )
    F_lw_values = F_lw(t_eval)
    F_sw_values = F_sw(t_eval)
    F = F_lw_values + F_sw_values
    N_lw = F_lw_values + sol.y[0] * lam_lw - lam0 * (sol.y[0] - sol.y[1])
    N_sw = F_sw_values + sol.y[0] * lam_sw
    N = N_sw + N_lw
    return xr.Dataset(
        dict(
            T_sfc=(
            ...
            F_lw=(
                "time",
                F_lw_values,
                {"long_name": "longwave forcing", "units": "W m-2"},
            ),
            F_sw=(
                "time",
                F_sw_values,
                {"long_name": "shortwave forcing", "units": "W m-2"},
            ),
